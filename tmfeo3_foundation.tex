...
=S_x+0.913\lambda^7+4.4\lambda^6+0.006\lambda^4+g_d\lambda^2$ with
...
$E_{12}$-excitation amplitude, and an emission avatar, calibrated by
the $(\qafm,E_{12})$ peak; only absolute-field calibration could
over-determine it), and the Fe:Tm emission efficiency $c_{\rm Fe}$,
bounded from \emph{above} by the non-observation of the
$(\qafm,\qafm)$ magnon diagonal (the atlas uses the bound value, at
which the diagonal sits at $0.14$, below the digitisation threshold,
while the $\lambda^6$-carried $(E_{12},\qafm)$ transfer is untouched:
$0.46\to0.43$).  The
$g_d\lambda^2$ member is the $ESJ$ vertex with its spin leg condensed
on $\langle F_x\rangle$: the one legal way an electric field reaches a
$\mathcal T$-odd quadrature, and the reason every $d_z^{\rm eff}$-fed
feature must collapse at the spin reorientation.
...
With the cross channels held fixed, two detection weights fitted once
(the CEF weight and the $d_z^{\rm eff}$ emission avatar) and the third
---the Fe weight $c_{\rm Fe}$---bounded from above by the absent
magnon diagonal, the overlap-gated delay apodisation applied (this
...
$(\qafm,E_{12})$ & $0.30$ & $0.34$ &
  magnon$\to$Tm, read through $d_z^{\rm eff}$ (reciprocity partner)\\
$(\qafm,\qafm)$ & absent & $0.14$ & magnon diagonal (Fe $F_x$, M1);
...
$(-E_{12},E_{23})$ & $<0.15$ & $\sim0.6$ & \textbf{open}: residual R1, point-ion floor\\
\bottomrule
\end{tabular}
\end{center}
...
Two residuals survive the propagation-completed model, each named and
characterised.  \emph{R1, the rephasing remnant} $(-E_{12},E_{23})$:
the point-ion qutrit has an exact floor of $0.47$--$0.58$ of the
dominant transfer (verified against a standalone three-level
integrator); no coupling in the exhausted vertex space, no drive
quadrature or sign (both scanned: exact nulls), and no linear filter
symmetric in $\pm\omega_\tau$ removes it.  \emph{R2, the marked
$(\qafm,1.25)$ peak}: the only symmetry-allowed magnon$\to$$E_{13}$
route, $\kappa^E_{5y}$ of the $ESJ$ family (previously untested),
produces it but floods the $1\!\leftrightarrow\!2$ block at any useful
strength---at $\kappa^E_{5y}=0.003$ already $(\qafm,E_{12})$ moves
$0.32\to0.57$ while the reabsorbed peak itself stays at $0.05$.
The emission side is exhausted as well: the Tm 4c site admits exactly
four quadratic moments beyond the measured $\beta\,\lambda^1\lambda^2$
---the electric $d_z$ pairs $\lambda^1\lambda^6$ and
$\lambda^2\lambda^7$ (mirror-odd, $\mathcal T$-even) and the magnetic
$m_x$ pairs $\lambda^1\lambda^7$ and $\lambda^2\lambda^6$ (mirror-odd,
$\mathcal T$-odd; M1, hence \emph{not} reabsorbed)---and every one
radiates at the sum frequency $E_{12}+E_{23}=1.20$\,THz.  Computed
directly from the production trajectories, all four product channels
share one structure: the $(E_{12},1.20)$ partner (stored $E_{12}$
coherence beating against the driven $\rho_{23}$) is $10$--$12\times$
the $(\qafm,1.20)$ target, whereas the measurement shows the opposite
ratio ($0.30$ against $\leq0.15$).  No linear channel and no quadratic
moment reproduces the peak without a dominant partner that is not
observed.
Matching R2 in-model would require the sequential $1\to2\to3$ cascade
to be $\sim\!15\times$ weaker than ideal three-level dynamics: physics
of the real third level beyond the qutrit truncation (its different
spatial character, faster dephasing, or hybridisation).  Both
residuals, the self-reversal wing amplitudes, and the geometry-B
...
\item \textbf{The quadratic-moment bound.}  The $4c$ site admits
  ...
  $(E_{12},1.20)$ therefore bounds all four at once; conversely, a
  future observation of a strong $(E_{12},1.20)$ peak, in either
  same-polarised channel, would revive them and reopen the R2
  assignment.
\end{enumerate}
