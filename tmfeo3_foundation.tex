...
One boundary of the shared-scale statement deserves precision.  The
cross-family scales are calibrated within geometry A and connect its
two channels exactly; transplanting them \emph{between geometries}
would additionally require the absolute internal field and the
polarisation-resolved transmission at each entry face, which
ratio-based calibration cannot supply.  The data decide the question
empirically: carrying the A-calibrated scales literally into the
geometry-B cross channel would make its map Tm-dominated (the
$\lambda^2$ and SFG/DFG features at $0.6$--$1.0$ of maximum), whereas
the measured channel is magnon-dominated with no Tm feature above
$\sim0.15$.  The magnon dominance of the measured B-cross map is
therefore one more null measurement: the geometry-B Tm-emission
normalisation lies at least $\sim5\times$ below the naive
A-transfer---consistent with the different entry polarisation and the
same optical-depth physics into which every remaining absolute-scale
question funnels.  At whatever scale that bound permits, the
un-truncated moment predicts the B-side SFG/DFG tones at
$(\qafm,0.40)$ and $(\qafm,1.41)$ in this channel---a low-level
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{paper/figs/atlas_final.pdf}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{paper/figs/samepol_darkmu13_final.pdf}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{paper/figs/samepol_vs_experiment.pdf}
\caption{Digitised measurement (a) against the model with the minimal
operator (b), same axes and colour scale.  Crosses mark the two
designated (``$+$'') $\qafm$-row peaks; the model places the high
tone at $\qafm{+}E_{12}=1.41$\,THz (the digitisation, at
$\pm0.05$\,THz, had anchored it to the nearby $E_{13}$ guide).
Six of nine amplitudes agree within $15\%$ (\cref{sec:samepolfit}).}
\label{fig:found-vsexp}
\end{figure*}

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{paper/figs/tm_dipole_forced.pdf}
\caption{Why the Tm dipole emits at the magnon frequency: an
off-resonant \emph{forced} response through the static family, not
harmonic generation.  (a)~Pump-only trajectories of $\lambda^2$ and
$F_x$ (offset): the dipole rings at its own $E_{12}$ with a small
$0.90$\,THz component on top.  (b)~The $0.90$\,THz content of
$\lambda^2$ grows linearly with $W^{xz}_1$ through the origin
(dashed: exact linearity).  (c)~The $\lambda^2$ spectrum: the sideband
sits at the magnon frequency, distinct from the $2E_{12}=1.00$
harmonic; the weak feature near $1.8$\,THz is the predicted
$2\qafm$ line.}
...
\subsection{Code-level verification of the SU(2)$\,\otimes\,$SU(3) implementation}
\label{sec:codeaudit}

Independently of the physics audit, the solver implementation was
verified against the theory of the preceding sections, item by item.

\emph{Algebra and equations of motion.}  All nine nonzero Gell-Mann
structure-constant triples are implemented at their standard values
($f_{123}=1$; $f_{147},f_{246},f_{257},f_{345}=\tfrac12$;
$f_{156},f_{367}=-\tfrac12$; $f_{458},f_{678}=\tfrac{\sqrt3}{2}$),
and the qutrit evolves under
$\dot\lambda_a=f_{abc}B_b\lambda_c$ with the onsite projection
$B_3=\varepsilon_2$, $B_8=(2\varepsilon_3-\varepsilon_2)/\sqrt3$ of
${\rm diag}(0,\varepsilon_2,\varepsilon_3)$: the three coherence
frequencies then come out analytically at $\varepsilon_2$,
$\varepsilon_3-\varepsilon_2$, $\varepsilon_3$, and numerically the
pump-only trajectories ring at $0.503$ ($1\!\leftrightarrow\!2$,
weakly dressed), $0.700$ ($2\!\leftrightarrow\!3$, visible in the
early window before its fast damping empties it) and $1.200$\,THz
($1\!\leftrightarrow\!3$, cascade-written)---the exact von Neumann
dynamics claimed in Sec.~S1.  The Fe sector integrates
Landau--Lifshitz--Gilbert with the standard cross-product kernel, and
one gradient convention ($H=+\partial E/\partial S$ against
$\dot S=H\times S$) is used consistently across Zeeman, anisotropy,
bilinear and trilinear terms.

\emph{Symmetry enforced structurally.}  The coupling parsers make the
$\mathcal T$-classes unrepresentable rather than merely unused: the
$SS\lambda$ reader exposes only the $\mathcal T$-even channels
$\lambda^{1,3,4,6,8}$, and the $K^-$ reader only the $\mathcal T$-odd
$\lambda^{2,5,7}$---a symmetry-violating vertex cannot even be
written into a parameter file.  The transported-gauge machinery
matches Secs.~S2--S3 exactly: Klein-four frames
${\rm diag}(\pm1,\pm1,\pm1)$ conjugate every $W$ tensor into its Fe
sublattice frame, the inversion-partner column signs are
${\rm diag}(+,-,-)$ on $(\lambda^2,\lambda^5,\lambda^7)$ for $K^-$,
flips on $\lambda^{4},\lambda^{6}$ for the $\lambda^+$ family, and
$-M_-$ for the inversion-odd electric ($ESJ$) legs.  The
field-assisted vertices are gated by the \emph{lab-frame} field
components through per-axis envelope tags, which is what makes
$\kappa^B$ bitwise inert in both measurement geometries.

\emph{Energy-consistent coupling.}  The trilinear
$S^TWS\,\lambda$ is distributed to all three sites as the exact
gradient of one energy: the Tm site receives
$\partial E/\partial\lambda = S^TWS$ once, and the Fe site receives
both spin legs ($2WS\,\lambda$ for the symmetric on-site pair), so
the forward (magnon$\to$CEF) and backward (CEF$\to$magnon)
conversions follow from one Hamiltonian term with no independent
back-action parameter.  The $\lambda$ leg couples to the deviation
from the thermal seed, so static CEF populations do not renormalise
the separately calibrated magnon spectrum.

\emph{Gauge uniformity, machine-exact.}  The staggered
($+,-,+,-$) four-sublattice combination of the Tm coordinates is
zero to the last floating-point digit---statically and at every
delay, in both geometries, through the full driven nonlinear
dynamics.  The four transported Tm ions are \emph{exactly}
equivalent, so the broadcast local-frame drive and the raw
sublattice-summed readout are exactly self-consistent, and any
physical sublattice projection sign structure can only multiply each
polarisation channel by an overall constant---absorbed by
construction into the calibrated cross-family scales, never into
ratios or positions.  The measured presence of all four channels at
model-consistent relative amplitudes confirms a posteriori that no
physical channel suffers a projection cancellation.

\emph{Ground state and protocol.}  The equilibrated reference is the
exact $\Gamma_2$ configuration ($F_x$ canting $8.4\times10^{-3}$ per
spin, staggered $G_z$ at full length $2.5$); the stored reference
trajectory is the pump-only $M_0$, so the extracted signal is
literally $\Mnl=M_{AB}-M_A-M_B$ with the static offset removed by
the analysis window; and the Boltzmann ensemble is implemented as
per-seed equilibria with Bloch damping toward the seed, the
$\lambda^3$ channel chasing the thermally shifted equilibrium of the
incoherent heating model.
