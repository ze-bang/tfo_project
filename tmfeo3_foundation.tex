...
while the $\lambda^6$-carried $(E_{12},\qafm)$ transfer is untouched).
...
$(E_{12},\qfm)$ & $0.45$ & $0.40$ & Tm$\to$magnon, $\qfm$ sideband on
  the $\lambda^6$ ($d_z$) line\\
$(E_{12},\qafm)$ & $0.40$ & $0.43$ & Tm$\to$magnon, carried as the
  magnon sideband on the $\lambda^6$ ($d_z$) line\\
...
this channel except the diagonal is carried by the Tm electric dipole:
the launched magnon modulates the crystal field and rides out as a
sideband on the strong $\lambda^6$ ($d_z$) line, so reducing the Fe
weight fourfold moves $(E_{12},\qafm)$ only from $0.46$ to $0.43$.
...
\begin{enumerate}
\item \textbf{Emission fingerprint and linewidth.}  The linewidth
  along $\omega_t$ names the emitter.  Geometry-B magnon emission (Fe
  $m_x$, M1) carries the magnon linewidth, $15\times$ sharper than a
  CEF line.  The geometry-A same-polarised magnon-frequency features,
  being sidebands on the Tm $d_z$ line, must instead carry the
  \emph{CEF} linewidth---and any recovered $(\qafm,\qafm)$ diagonal in
  that channel would raise the $c_{\rm Fe}$ bound and must then appear
  with the magnon width.
