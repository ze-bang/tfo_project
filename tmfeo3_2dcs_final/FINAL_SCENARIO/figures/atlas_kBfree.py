...
def load(run,sp,l):
    ...
    return t,tau,M
def spec(t,tau,M,samepol=False,subtract=True):
    tm=t>=3.0
    if samepol:
    ...
    Md=((M[:,tm]-M[:,tm].mean(axis=0,keepdims=True)) if subtract else M[:,tm])*atau[:,None]*at[None,:]
...
def blind(wtb,wTb,A,amp_min=0.05):
    ...
    return out[:8]
def mix(runs,sp,l,T,samepol=False,subtract=True):
    ...
    return spec(t,tau,tot,samepol,subtract)
A2=[f"FINAL_SCENARIO/runs/geomA_{g}" for g in ["gs1","gs2","gs3"]]   # geometry A: ONE drive for both its channels
B =["FINAL_SCENARIO/runs/geomB_gs1"]
...
wtb,wTb,a=mix(A2,"SU3",1,10,subtract=False); _,_,b=mix(A2,"SU2",2,10,subtract=False)
_,_,a1=mix(A2,"SU3",0,10,subtract=False); _,_,a3=mix(A2,"SU3",2,10,subtract=False)
