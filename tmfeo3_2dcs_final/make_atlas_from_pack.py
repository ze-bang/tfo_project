...
PACK = sys.argv[1] if len(sys.argv) > 1 else f"{HERE}/FINAL_SCENARIO/trajectories/atlas_construction_pack.h5"
...
fig.tight_layout()
fig.savefig(f"{HERE}/FINAL_SCENARIO/atlas_from_pack.pdf", dpi=300)
fig.savefig(f"{HERE}/FINAL_SCENARIO/atlas_from_pack.png", dpi=120)
json.dump(census, open(f"{HERE}/FINAL_SCENARIO/census_from_pack.json", "w"), indent=1)
